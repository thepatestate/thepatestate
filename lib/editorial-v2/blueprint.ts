...
export async function buildBlueprint(dossier: EditorialDossier, angle: StoryAngle, decision: AngleDecision, opts: { product: string; guidance?: string }): Promise<{ blueprint: StoryBlueprint; call: StageCall }> {
...
export async function reviewBlueprint(dossier: EditorialDossier, blueprint: StoryBlueprint, angle: StoryAngle): Promise<{ review: BlueprintReview; call: StageCall }> {
  const { data, call } = await callJSON<BlueprintReview>({
    stage: "blueprint-editor", role: "blueprintEditor", maxTokens: 9000,
