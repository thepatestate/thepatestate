...
const AB = { type: "string", enum: ["A", "B", "neither"] };
const pick = obj({ winner: AB, reason: S });

export const SELECTION_SCHEMA = obj({
  winner: { type: "string", enum: ["A", "B", "hybrid", "neither"] },
  opening: pick, argument: pick, football: pick, audienceConnection: pick,
  bestParagraphs: arr(obj({ draft: { type: "string", enum: ["A", "B"] }, paragraphIndex: { type: "integer" }, reason: S })),
  cut: arr(obj({ draft: { type: "string", enum: ["A", "B"] }, paragraphIndex: { type: "integer" }, reason: S })),
...
export async function editDrafts(drafts: WriterOutput[], blueprint: StoryBlueprint, thesis: string): Promise<{ selection: DraftSelection; call: StageCall }> {
  const a = drafts.find((d) => d.writer === "A");
  const b = drafts.find((d) => d.writer === "B");
  ...
  const { data, call } = await callJSON<DraftSelection>({
    ...
    user: `THESIS: ${thesis}\n\n${blueprintBlock(blueprint)}\n\nDRAFT A${a ? ` — ${a.draft.headline}\n${a.draft.dek}\n\n${numbered(a.draft.bodyMarkdown)}` : ": (writer failed; treat as absent)"}\n\nDRAFT B${b ? ` — ${b.draft.headline}\n${b.draft.dek}\n\n${numbered(b.draft.bodyMarkdown)}` : ": (writer failed; treat as absent)"}`,
  });
