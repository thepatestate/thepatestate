...
/** V3: the same independent gate against plain source text (a transcript
 * segment, outlet text, the fact sheet). Sources are the factual universe. */
export async function factCheckSources(draft: ArticleDraft, sources: string): Promise<{ result: FactCheckResult; call: StageCall }> {
  const started = Date.now();
  const anthropic = process.env.ANTHROPIC_API_KEY ? new Anthropic() : null;
  const { text, via } = await judgeJSON(anthropic, {
    maxTokens: 6000, effort: "low", schemaName: "fact_check", schema: FACTCHECK_SCHEMA as unknown as Record<string, unknown>,
    system: v2Prompt("fact-check"),
    user: `SOURCES:\n${sources.slice(0, 60000)}\n\nDRAFT:\nHEADLINE: ${draft.headline}\nDEK: ${draft.dek}\n\n${draft.bodyMarkdown}\n\nPULL QUOTE: ${draft.pullQuote}`,
  });
  let result: FactCheckResult;
  try { result = JSON.parse(text) as FactCheckResult; } catch { result = { verdict: "unsupported", claims: [], joshMisattribution: ["fact-check response unreadable"] }; }
  // A "contradicted" that cannot quote the contradicting source sentence is
