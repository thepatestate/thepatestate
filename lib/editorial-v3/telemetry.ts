...
/** V3 runs share the table: engine in `lane`, product `v3-${engine}`. */
export async function recordV3Run(run: V3Run): Promise<void> {
  const clean = scrub(run);
  try { mkdirSync(localRunDir(), { recursive: true }); writeFileSync(join(localRunDir(), `${run.id}.json`), JSON.stringify(clean, null, 2)); } catch (err) { console.warn("[v3:telemetry] local write failed", err instanceof Error ? err.message : err); }
  if (!isAdminConfigured || process.env.VITEST) return;
  try {
    const { error } = await createAdminClient().from("editorial_runs").upsert({
      id: run.id, lane: run.engine, product: `v3-${run.engine}`, source_id: run.sourceId, fixture: run.fixture ?? null, mode: run.mode,
      status: run.status === "failed" ? "failed" : "completed", started_at: run.startedAt, completed_at: run.completedAt ?? null, cycles: run.artifacts.repairs?.length ?? 0,
      decision: run.status === "no-article" ? "no-article" : run.artifacts.quit?.didFinish === false ? "hold" : "accept", failure_class: run.artifacts.quit?.reason ?? null,
      final_score: null, published_content_id: null, total_cost_usd: run.totalCostUsd, total_calls: run.calls.length, artifacts: clean, error: run.error ?? null,
    }, { onConflict: "id" });
    if (error) console.warn("[v3:telemetry] upsert failed", error.message);
  } catch (err) { console.warn("[v3:telemetry] upsert threw", err instanceof Error ? err.message : err); }
}
