...
// from its own sources with the same _id, slug and publishedAt; a story the
// desk gate rejects is deleted with its item (a national desk would not run
// it). Every replaced or deleted document is snapshotted first.
...
type S = { _id: string; headline: string; publishedAt: string; category?: string; teams?: string[]; whatHappened?: string; sources?: { outlet?: string; url?: string }[]; item: { _id: string; sourceUrls?: string[]; sourceOutlets?: string[] } | null };
const stories = await writeClient.fetch<S[]>(`*[_type=="wireStory" && !(_id in path("drafts.**")) && publishedAt >= $since && publishedAt < $until] | order(publishedAt asc){ _id, headline, publishedAt, category, teams, whatHappened, sources, "item": *[_type=="wireItem" && references(^._id)][0]{ _id, sourceUrls, sourceOutlets } }`, { since: SINCE, until: UNTIL });
say(`Wire stories ${SINCE} → ${UNTIL}: ${stories.length}${DRY ? " (dry run)" : ""}`);
const counts = { rewritten: 0, deleted: 0, skipped: 0, cost: 0 };
async function one(s: S) {
  ...
  const v3 = await v3WireStory({ clusterKey: s._id.replace(/^wireStory-/, ""), teams: s.teams ?? [], refs, category: s.category, mode: "live", tier: "economy", gate: true });
  if (v3.run) counts.cost += v3.run.totalCostUsd;
  if (!v3.ok && /^desk gate/.test(v3.reason)) {
    say(`DELETE  ${s.publishedAt.slice(5, 16)}  ${s.headline.slice(0, 70)}  — ${v3.reason.slice(0, 90)}`);
...
say(`\nDONE rewritten ${counts.rewritten} · deleted ${counts.deleted} · skipped ${counts.skipped} · model cost $${counts.cost.toFixed(2)}`);
